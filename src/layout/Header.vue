...
export default{
  ...
  data() {
    ...
    return {
      useRoute,
      homePageAddress:"/",
      productsPageAddress:"/productPage"
    }
  },
  methods:{
    toMainPage(){
...
<template>
  <header>
    <nav>
      <svg width="110" height="28" viewBox="0 0 110 28" fill="none" xmlns="http://www.w3.org/2000/svg" @click="toMainPage()">
      ...
      <router-link to="/" class="link" :class="{['link_active'] : getPagePath === homePageAddress}">Home</router-link>
      <router-link to="/productPage" class="link" :class="{['link_active'] : getPagePath === productsPageAddress}">Products</router-link>
    </nav>
  </header>
</template>
